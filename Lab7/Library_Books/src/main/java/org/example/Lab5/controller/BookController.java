...
import org.example.Lab5.model.*;
import org.example.Lab5.service.*;
...
public class BookController
{
    ...
    @Autowired
    public BookController(BookService bookService, SimplifiedBookGenreService bookGenreService)
    {
    ...
    {
        SimplifiedBookGenre genre = bookGenreService.findById(bookDto.getGenreId())
    ...
    public ResponseEntity<?> getBooksByGenreId(@PathVariable UUID genreId)
    {
        Optional<SimplifiedBookGenre> genreOptional = bookGenreService.findById(genreId);

        if (genreOptional.isEmpty())
        {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("Genre not found with ID: " + genreId);
        }

        List<BookCollectionDTO> books = bookService.findByBookGenre(genreOptional.get()).stream()
    ...
    public ResponseEntity<?> updateBook(@PathVariable UUID bookId, @RequestBody BookCreateUpdateDTO bookDto)
    {
        ...
        Optional<SimplifiedBookGenre> genreOptional = bookGenreService.findById(bookDto.getGenreId());
        if (genreOptional.isEmpty())
        {
